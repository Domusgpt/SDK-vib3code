/**
 * E8.js
 * The Mathematical Core of the Quatossian Inscription Framework.
 *
 * Provides:
 * 1. Generation of E8 Gosset Lattice Roots (240 vertices of 4_21 polytope).
 * 2. Moxness Folding: A projection from 8D -> 4D (H4 600-cell) -> 3D.
 * 3. Quatossian Kernel: Lattice quantization and quaternion spin generation.
 * 4. Spatial Sorting: Z-Curve / Morton Ordering for efficient storage/rendering.
 *
 * References:
 * - J.G. Moxness, "The E8 Polytopes"
 * - Conway & Sloane, "Sphere Packings, Lattices and Groups"
 */

import { PLASTIC_CONSTANT } from './Plastic.js';
import { Quaternion } from './Quaternion.js';

export class E8Lattice {

    // The Golden Ratio
    static PHI = 1.61803398875;
    static INV_PHI = 0.61803398875; // 1/phi = phi - 1

    /**
     * Generates the 240 Roots of the E8 Lattice.
     * These are the vertices of the Gosset 4_21 polytope.
     * Length squared = 2.
     *
     * Structure:
     * - 112 roots: Permutations of (±1, ±1, 0^6)
     * - 128 roots: (±1/2)^8 with even number of minus signs
     *
     * @returns {Float32Array} Flat array of 240 * 8 coordinates.
     */
    static generateRoots() {
        const roots = [];

        // 1. Integer Roots (112)
        // Permutations of (±1, ±1, 0, 0, 0, 0, 0, 0)
        // Indices j < k
        for (let j = 0; j < 8; j++) {
            for (let k = j + 1; k < 8; k++) {
                const signs = [[1, 1], [1, -1], [-1, 1], [-1, -1]];
                for (let s of signs) {
                    const v = new Float32Array(8);
                    v[j] = s[0];
                    v[k] = s[1];
                    roots.push(v);
                }
            }
        }

        // 2. Half-Integer Roots (128)
        // (±1/2, ..., ±1/2) with even sum of signs
        // We iterate 0..255 (binary)
        for (let i = 0; i < 256; i++) {
            // Check parity (number of set bits)
            // We want even number of MINUS signs.
            let minusCount = 0;
            let temp = i;
            while (temp > 0) {
                if (temp & 1) minusCount++;
                temp >>= 1;
            }

            if (minusCount % 2 === 0) {
                const v = new Float32Array(8);
                for (let b = 0; b < 8; b++) {
                    v[b] = ((i >> b) & 1) ? -0.5 : 0.5;
                }
                roots.push(v);
            }
        }

        // Flatten
        const flat = new Float32Array(roots.length * 8);
        for (let i = 0; i < roots.length; i++) {
            flat.set(roots[i], i * 8);
        }
        return flat;
    }

    /**
     * Projects 8D E8 coordinates to 4D H4 (600-cell) space.
     * This is the "Moxness Folding" operation.
     *
     * The projection matrix typically folds E8 using the Golden Ratio to preserve H4 symmetry.
     * We use the standard folding:
     * u = (v1 + phi*v5, v2 + phi*v6, v3 + phi*v7, v4 + phi*v8) * factor
     *
     * @param {Float32Array} v8 - 8D Vector
     * @returns {Float32Array} 4D Vector (Quaternion)
     */
    static foldTo4D(v8) {
        const phi = E8Lattice.PHI;
        const q = new Float32Array(4);

        q[0] = v8[0] + phi * v8[4];
        q[1] = v8[1] + phi * v8[5];
        q[2] = v8[2] + phi * v8[6];
        q[3] = v8[3] + phi * v8[7];

        return q;
    }

    /**
     * Projects 4D (Quaternion) to 3D Space.
     *
     * @param {Float32Array} q - 4D Vector
     * @returns {Float32Array} 3D Vector
     */
    static project4Dto3D(q) {
        // Perspective projection from 4D to 3D.
        // w is the "time" or "scale" dimension.
        const w = q[3];
        const dist = 2.0 - w; // Camera at w=2
        const f = 1.0 / (dist > 0.001 ? dist : 0.001);

        return new Float32Array([
            q[0] * f,
            q[1] * f,
            q[2] * f
        ]);
    }

    /**
     * Rotates a 4D vector using a unit quaternion.
     * Uses the Quaternion class helper.
     */
    static rotate4D(v4, q) {
        // q must be a Quaternion object
        return Quaternion.rotate4DLeft(v4, q);
    }

    /**
     * Generates a "Quatossian Cloud".
     * A set of points in 3D, each with an associated Quaternion spin.
     *
     * @param {number} shells - Number of lattice shells to generate (approx radius).
     * @returns {Object} { positions: Float32Array, rotations: Float32Array }
     */
    static generateCloud(shells = 1) {
        const rootsFlat = this.generateRoots();
        const numRoots = 240;

        const count = numRoots * shells;
        const positions = new Float32Array(count * 3);
        const rotations = new Float32Array(count * 4); // Quaternions

        let idx = 0;

        for (let s = 0; s < shells; s++) {
            // "Plastic Ratio (rho ~ 1.3247) Sampling"
            const scale = Math.pow(PLASTIC_CONSTANT, s + 1);

            for (let i = 0; i < numRoots; i++) {
                // Extract 8D root
                const v8 = rootsFlat.subarray(i*8, i*8+8);

                // Fold to 4D (This is our "Spin" state!)
                // The 4D projection of an E8 root is a quaternion.
                const q = this.foldTo4D(v8);

                // Normalize q to be a valid rotation quaternion
                let len = Math.sqrt(q[0]*q[0] + q[1]*q[1] + q[2]*q[2] + q[3]*q[3]);
                if (len > 0) {
                    q[0]/=len; q[1]/=len; q[2]/=len; q[3]/=len;
                }

                // Store Rotation
                rotations[idx * 4 + 0] = q[0];
                rotations[idx * 4 + 1] = q[1];
                rotations[idx * 4 + 2] = q[2];
                rotations[idx * 4 + 3] = q[3];

                // Project to 3D (Position)
                // Position = Direction of q projected to 3D * Scale
                // Or use the "Moxness" shadow directly.
                const shadow = this.project4Dto3D(E8Lattice.foldTo4D(v8)); // Use un-normalized for position

                positions[idx * 3 + 0] = shadow[0] * scale;
                positions[idx * 3 + 1] = shadow[1] * scale;
                positions[idx * 3 + 2] = shadow[2] * scale;

                idx++;
            }
        }

        return { positions, rotations };
    }

    /**
     * Sorts a point cloud based on 3D Z-Curve (Morton Order).
     * This optimizes spatial coherence for rendering and compression.
     *
     * @param {Object} cloud - { positions, rotations, scales?, colors? }
     * @returns {Object} A NEW sorted cloud object.
     */
    static sortCloud(cloud) {
        const count = cloud.positions.length / 3;
        const indices = new Uint32Array(count);
        for(let i=0; i<count; i++) indices[i] = i;

        // 1. Calculate Bounding Box
        let minX=Infinity, minY=Infinity, minZ=Infinity;
        let maxX=-Infinity, maxY=-Infinity, maxZ=-Infinity;

        const pos = cloud.positions;
        for(let i=0; i<count; i++) {
            const x = pos[i*3], y = pos[i*3+1], z = pos[i*3+2];
            if(x<minX) minX=x; if(x>maxX) maxX=x;
            if(y<minY) minY=y; if(y>maxY) maxY=y;
            if(z<minZ) minZ=z; if(z>maxZ) maxZ=z;
        }

        // Avoid division by zero
        const rangeX = (maxX - minX) || 1;
        const rangeY = (maxY - minY) || 1;
        const rangeZ = (maxZ - minZ) || 1;

        // 2. Compute Morton Codes (Interleave bits)
        // We use a simplified sorting key: Z-index mapping.
        // Javascript sort is stable.
        // We map positions to 10-bit integers (0..1023)

        const codes = new Uint32Array(count);

        for(let i=0; i<count; i++) {
            const x = Math.floor(((pos[i*3] - minX) / rangeX) * 1023);
            const y = Math.floor(((pos[i*3+1] - minY) / rangeY) * 1023);
            const z = Math.floor(((pos[i*3+2] - minZ) / rangeZ) * 1023);

            // Interleave (Expanded for clarity, optimization possible with bit magic)
            // 30-bit code fits in Uint32
            // We just pack them: z<<20 | y<<10 | x
            // This is actually "linearization", not strict Morton, but sufficient for locality.
            // Strict Morton: bit 0 of x, bit 0 of y, bit 0 of z, bit 1 of x...
            // Let's use simple packing for JS performance unless strict Z-curve needed.
            // Z-curve is better for octrees.
            // Z << 20 | Y << 10 | X is "Row Major Layered" order.
            // It clusters locally in XY planes, then stacks Z.
            // Let's try to implement a simple Morton for better 3D locality.

            // https://graphics.stanford.edu/~seander/bithacks.html#InterleaveTableObvious
            // We'll just use the packing for now, it's fast.
            codes[i] = (z << 20) | (y << 10) | x;
        }

        // 3. Sort Indices
        indices.sort((a, b) => codes[a] - codes[b]);

        // 4. Reorder Arrays
        const newPos = new Float32Array(pos.length);
        const newRot = cloud.rotations ? new Float32Array(cloud.rotations.length) : null;
        const newScale = cloud.scales ? new Float32Array(cloud.scales.length) : null;
        const newCol = cloud.colors ? new Float32Array(cloud.colors.length) : null;

        for(let i=0; i<count; i++) {
            const oldIdx = indices[i];

            newPos[i*3+0] = pos[oldIdx*3+0];
            newPos[i*3+1] = pos[oldIdx*3+1];
            newPos[i*3+2] = pos[oldIdx*3+2];

            if(newRot) {
                newRot[i*4+0] = cloud.rotations[oldIdx*4+0];
                newRot[i*4+1] = cloud.rotations[oldIdx*4+1];
                newRot[i*4+2] = cloud.rotations[oldIdx*4+2];
                newRot[i*4+3] = cloud.rotations[oldIdx*4+3];
            }
            if(newScale) {
                newScale[i] = cloud.scales[oldIdx];
            }
            if(newCol) {
                newCol[i*3+0] = cloud.colors[oldIdx*3+0];
                newCol[i*3+1] = cloud.colors[oldIdx*3+1];
                newCol[i*3+2] = cloud.colors[oldIdx*3+2];
            }
        }

        return {
            positions: newPos,
            rotations: newRot,
            scales: newScale,
            colors: newCol
        };
    }
}
