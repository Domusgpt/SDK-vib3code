...
void main() {
    ...
    /**
     * Get the actual Faceted system fragment shader
     */
...
vec4 apply6DRotation(vec4 pos) {
    ...
    pos = rotateXW(u_rot4dXW + u_time * 0.07 + u_bass * 0.5) * pos;
    pos = rotateYW(u_rot4dYW + u_time * 0.08 + u_mid * 0.3) * pos;
    pos = rotateZW(u_rot4dZW + u_time * 0.09 + u_high * 0.4) * pos;
    return pos;
}
...
float hypertetrahedronCore(vec4 p, float baseType) {
    float tetraField = max(max(max(
        abs(p.x+p.y)-p.z-p.w, abs(p.x-p.y)-p.z+p.w),
        abs(p.x+p.y)+p.z-p.w), abs(p.x-p.y)+p.z+p.w) / 2.0;
...
void main() {
    ...
    uv *= 2.0 / (u_gridDensity * 0.1 + u_bass * 0.5);

    vec4 pos = vec4(uv, sin(u_time * 0.3) * 0.5, cos(u_time * 0.2) * 0.5);
    pos = apply6DRotation(pos);
    pos *= u_morphFactor + u_mid * 0.5;
    pos += vec4(sin(u_time*0.1),cos(u_time*0.15),sin(u_time*0.12),cos(u_time*0.18)) * (u_chaos + u_high * 0.3);

    float dist = geometry(pos, u_geometry);
    float edge = smoothstep(0.02, 0.0, abs(dist));
    float fill = smoothstep(0.1, 0.0, dist) * 0.3;

    float hueVal = u_hue / 360.0 + dist * 0.2 + u_time * 0.05 + u_mid * 0.2;
    vec3 color = vec3(
