class LookaheadBuffer {
  ...
  pushFrame(bands, energy, timestamp = performance.now()) {
    if (!Array.isArray(bands)) return;
    const normalizedBands = bands.map((value) => Number.isFinite(value) ? Math.max(0, Math.min(1, value)) : 0);
    const clampedEnergy = Number.isFinite(energy) ? Math.max(0, Math.min(1.5, energy)) : 0;

    this.frames.push({ bands: normalizedBands, energy: clampedEnergy, timestamp });
