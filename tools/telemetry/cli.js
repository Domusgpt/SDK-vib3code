#!/usr/bin/env node
import { readFile, stat, watch, writeFile } from 'node:fs/promises';
...
async function commandExportPack(filePath, packName = 'pack', outputPath = 'automation-export.json') {
  const raw = await readFile(filePath, 'utf8');
  const pack = JSON.parse(raw);
  ...
  const exported = director.exportPack(pack, { packName });
  await writeFile(outputPath, JSON.stringify(exported, null, 2));

  console.log(
    `Exported ${exported.assets.length} assets with ${exported.transitions.length} transitions to ${outputPath} for pack ${packName}`
  );
  console.log(`Controls covered: ${exported.stats.controls.covered.length}`);
  if (exported.stats.controls.missing.length) {
    console.warn(`Missing controls: ${exported.stats.controls.missing.join(', ')}`);
  }
  console.log('✅ pack exported');
}
