...
- **Audio choreographer + uploads:** live reactivity now runs through a JusDNCE-inspired choreographer (256-size FFT, 0.8 smoothing, tri-band gating, and 200ms lookahead) that emits heartbeat/snap cues to the renderer. The 🧪 overlay adds mic/oscillator/file-upload toggles plus sequence mode (hybrid/environment/character) so you can feed the visuals from recorded tracks as well as live input.
