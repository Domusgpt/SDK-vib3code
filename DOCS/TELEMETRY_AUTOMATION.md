...
- Export a pack into a reusable asset manifest that lists every state, transition, and covered control: `node tools/telemetry/cli.js export-pack tests/fixtures/automation-pack-valid.json fixtures-demo docs/fixtures-demo-export.json`
...
### Asset manifests for galleries and packs

`exportAutomationPack` (browser) and `pnpm telemetry:export-pack` (CLI) convert a pack into an agent-friendly manifest that lists:

- `states`: every preset with controls/reactivity/audio payloads and optional author notes.
- `transitions`: the sweep/hold steps extracted from sequences so you can preview how visuals evolve.
- `assets`: a flat list of variant IDs pointing back to the source state, suitable for galleries or batch exports.

Example:

```bash
pnpm telemetry:export-pack -- tests/fixtures/automation-pack-valid.json fixtures-demo docs/fixtures-demo-export.json
cat docs/fixtures-demo-export.json
```

```json
{
  "pack": "fixtures-demo",
  "stats": { "states": 2, "sequences": 1, "rules": 1, "steps": 2, "estimatedDurationMs": 450, "modulators": 1, "triggers": 1, "controls": { "covered": ["rot4dXY", "speed", "hue"], "missing": [] } },
  "assets": [
    { "id": "fixtures-demo-base", "state": "base", "summary": { "controls": ["rot4dXY", "speed", "hue"] } },
    { "id": "fixtures-demo-burst", "state": "burst", "summary": { "controls": ["rot4dXY", "speed", "hue"] } }
  ],
  "transitions": [ { "sequence": "demo", "from": null, "to": "base", "holdMs": 150 }, { "sequence": "demo", "from": "base", "to": "burst", "sweep": { "durationMs": 300, "easing": "linear" } } ]
}
```

Ship the manifest with your style packs so agents can regenerate or preview the same asset families without opening the canvas.
